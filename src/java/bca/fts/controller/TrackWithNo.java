// this servlet is responsible for getting the file-track data for a branch via file_no and show it via VIEW
...
public class TrackWithNo extends HttpServlet {

    // the logger object
    private static final Logger logger = Logger.getLogger(TrackWithNo.class);
    ...
    private void forwardData(TrackFileBean file, HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("trackWithNo.jsp").forward(request, response);
    }
        ...
        if (file_id != null) {
            ...
            if (no == null) {
                ...
                if (no.isEmpty()) {
                    ...
                    request.setAttribute("errorMessage", "Please enter the file no");
                    request.getRequestDispatcher("trackWithNo.jsp").forward(request, response);
                ...
                if (isAllOk) {
                    ...
                    logger.info(user + " searched for a file having No " + no);

                    files = BranchFileTrackDao.giveFileRecordViaNo(no.trim(), branch.getOfficeName());
                    ...
                    if (files == null) {
                        ...
                        request.setAttribute("errorMessage", "Request is denied. You haven't process the file yet");
                        request.getRequestDispatcher("trackWithNo.jsp").forward(request, response);
