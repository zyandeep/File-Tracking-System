// this servlet is responsible for getting the file-track data from DAO via file_no show it via VIEW
...
public class TrackNoAdmin extends HttpServlet {

    // the logger object
    private static final Logger logger = Logger.getLogger(TrackNoAdmin.class);
    ...
    private void forwardData(TrackFileBean file, HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("trackNoAdmin.jsp").forward(request, response);
    }
        ...
        if (file_id != null) {
            ...
            if (no == null) {
                ...
                if (isAllOk) {
                    ...
                    logger.info(user + " searched a file by No: " + no);

                    files = AdminFileTrackDao.giveFilesViaNo(no.trim());
