// this servlet is responsible for getting the file-track data for a branch via file name and show it via VIEW
...
public class TrackWithName extends HttpServlet {

    // the logger object
    private static final Logger logger = Logger.getLogger(TrackWithName.class);
    ...
    private void forwardData(TrackFileBean file, HttpServletRequest request, HttpServletResponse response)
        ...
        request.getRequestDispatcher("trackWithName.jsp").forward(request, response);
    }
        ...
        if (file_id != null) {
            ...
            if (name == null) {
                ...
                if (isAllOk) {
                    ...
                    logger.info(user + " searched for a file having name " + name);

                    files = BranchFileTrackDao.giveFileRecordViaName(name.trim(), branch.getOfficeName());
